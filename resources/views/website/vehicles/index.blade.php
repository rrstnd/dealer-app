...
<body class="bg-slate-50 text-slate-800 antialiased selection:bg-blue-600 selection:text-white min-h-screen flex flex-col justify-between">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-20">
                ...
                <nav class="hidden md:flex items-center gap-6">
                    ...
                    <a href="{{ route('vehicles.index', ['vehicle_type_id' => 1]) }}" class="text-sm font-semibold {{ request('vehicle_type_id') == 1 ? 'text-blue-600' : 'text-slate-600 hover:text-slate-900' }} transition">Katalog Mobil</a>
                    <a href="{{ route('vehicles.index', ['vehicle_type_id' => 2]) }}" class="text-sm font-semibold {{ request('vehicle_type_id') == 2 ? 'text-blue-600' : 'text-slate-600 hover:text-slate-900' }} transition">Katalog Motor</a>
                    <a href="{{ route('home') }}#features" class="text-sm font-medium text-slate-600 hover:text-slate-900 transition">Keunggulan</a>
                    <a href="#contact" class="text-sm font-medium text-slate-600 hover:text-slate-900 transition">Hubungi Kami</a>
                </nav>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full flex-grow space-y-6">
        ...
        {{-- SEARCH BAR & SORTING --}}
        <form method="GET" action="{{ route('vehicles.index') }}" class="flex flex-col sm:flex-row items-center justify-between gap-4">
            {{-- SEARCH INPUT --}}
            <div class="relative w-full sm:w-96">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari merek, model, atau tipe mobil..."
                       class="w-full pl-10 pr-4 py-2.5 text-xs bg-white border border-slate-200 rounded-2xl shadow-sm focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600 text-slate-800 placeholder-slate-400">
            </div>

            {{-- SORT DROPDOWN --}}
            <div class="flex items-center gap-2 text-xs text-slate-600 w-full sm:w-auto justify-end">
                <span>Urutkan:</span>
                <select name="sort" onchange="this.form.submit()" class="bg-white border border-slate-200 text-slate-800 text-xs font-semibold rounded-2xl px-4 py-2 shadow-sm focus:outline-none focus:border-blue-600">
        ...
        {{-- CONTENT LAYOUT: SIDEBAR FILTER + GRID --}}
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
            
            {{-- SIDEBAR FILTER --}}
            <aside class="bg-white border border-slate-200/80 p-6 rounded-3xl shadow-sm space-y-6">
                <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                    <h2 class="font-extrabold text-slate-900 text-sm flex items-center gap-2">
                        <span>🎛️</span> Filter Unit <span class="text-xs font-normal text-slate-400">({{ $vehicles->total() }})</span>
                    </h2>
                    @if(request()->hasAny(['search', 'brand_id', 'transmission', 'fuel_type', 'year_min', 'price_max', 'sort']))
                        <a href="{{ route('vehicles.index') }}" class="text-[11px] font-semibold text-rose-500 hover:underline">Reset</a>
                    @endif
                </div>
                ...
                <form method="GET" action="{{ route('vehicles.index') }}" class="space-y-6">
                    ...
                    <div>
                        <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2.5">Kategori Kendaraan</label>
                        <div class="grid grid-cols-3 gap-1.5">
                            <a href="{{ route('vehicles.index', array_merge(request()->except('vehicle_type_id'), ['vehicle_type_id' => ''])) }}"
                               class="text-center px-2 py-1.5 rounded-xl text-xs font-semibold border transition {{ !request('vehicle_type_id') ? 'bg-blue-600 text-white border-blue-600' : 'bg-slate-50 text-slate-600 border-slate-200 hover:bg-slate-100 hover:text-slate-900' }}">
                                Semua
                            </a>
                            @foreach ($vehicleTypes as $type)
                                <a href="{{ route('vehicles.index', array_merge(request()->except('vehicle_type_id'), ['vehicle_type_id' => $type->id])) }}"
                                   class="text-center px-2 py-1.5 rounded-xl text-xs font-semibold border transition {{ request('vehicle_type_id') == $type->id ? 'bg-blue-600 text-white border-blue-600' : 'bg-slate-50 text-slate-600 border-slate-200 hover:bg-slate-100 hover:text-slate-900' }}">
                                    {{ $type->name }}
                                </a>
                            @endforeach
                        </div>
                    ...
                    {{-- MEREK KENDARAAN (PILLS) --}}
                    <div>
                        <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2.5">Merek Kendaraan</label>
                        <div class="flex flex-wrap gap-1.5">
                            <a href="{{ route('vehicles.index', array_merge(request()->except('brand_id'), ['brand_id' => ''])) }}"
                               class="px-3 py-1.5 rounded-xl text-xs font-semibold border transition {{ !request('brand_id') ? 'bg-slate-900 text-white border-slate-900' : 'bg-slate-50 text-slate-600 border-slate-200 hover:bg-slate-100 hover:text-slate-900' }}">
                                Semua
                            </a>
                            @foreach ($brands as $brand)
                                <a href="{{ route('vehicles.index', array_merge(request()->except('brand_id'), ['brand_id' => $brand->id])) }}"
                                   class="px-3 py-1.5 rounded-xl text-xs font-semibold border transition {{ request('brand_id') == $brand->id ? 'bg-slate-900 text-white border-slate-900' : 'bg-slate-50 text-slate-600 border-slate-200 hover:bg-slate-100 hover:text-slate-900' }}">
                                    {{ $brand->name }}
                                </a>
                            @endforeach
                        </div>
                    ...
                    {{-- TRANSMISI (PILLS) --}}
                    <div>
                        <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2.5">Transmisi</label>
                        <div class="grid grid-cols-2 gap-1.5">
                            @foreach(['CVT' => 'CVT', 'Otomatis' => 'Otomatis', 'Manual' => 'Manual'] as $key => $label)
                                <a href="{{ route('vehicles.index', array_merge(request()->except('transmission'), ['transmission' => request('transmission') == $key ? '' : $key])) }}"
                                   class="text-center px-3 py-1.5 rounded-xl text-xs font-semibold border transition {{ request('transmission') == $key ? 'bg-slate-900 text-white border-slate-900' : 'bg-slate-50 text-slate-600 border-slate-200 hover:bg-slate-100 hover:text-slate-900' }}">
                                    {{ $label }}
                                </a>
                            @endforeach
                        </div>
                    ...
                    {{-- BAHAN BAKAR (PILLS) --}}
                    <div>
                        <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2.5">Bahan Bakar</label>
                        <div class="flex gap-1.5">
                            @foreach(['Bensin' => 'Bensin', 'Diesel' => 'Diesel'] as $key => $label)
                                <a href="{{ route('vehicles.index', array_merge(request()->except('fuel_type'), ['fuel_type' => request('fuel_type') == $key ? '' : $key])) }}"
                                   class="flex-1 text-center px-3 py-1.5 rounded-xl text-xs font-semibold border transition {{ request('fuel_type') == $key ? 'bg-slate-900 text-white border-slate-900' : 'bg-slate-50 text-slate-600 border-slate-200 hover:bg-slate-100 hover:text-slate-900' }}">
                                    {{ $label }}
                                </a>
                            @endforeach
                        </div>
                    ...
                    {{-- TAHUN MINIMAL (PILLS) --}}
                    <div>
                        <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2.5">Tahun Minimal</label>
                        <div class="grid grid-cols-2 gap-1.5">
                            @foreach(['2022' => '≥ 2022', '2021' => '≥ 2021', '2020' => '≥ 2020', '2019' => '≥ 2019'] as $yr => $lbl)
                                <a href="{{ route('vehicles.index', array_merge(request()->except('year_min'), ['year_min' => request('year_min') == $yr ? '' : $yr])) }}"
                                   class="text-center px-2 py-1.5 rounded-xl text-xs font-semibold border transition {{ request('year_min') == $yr ? 'bg-slate-900 text-white border-slate-900' : 'bg-slate-50 text-slate-600 border-slate-200 hover:bg-slate-100 hover:text-slate-900' }}">
                                    {{ $lbl }}
                                </a>
                            @endforeach
                        </div>
                    ...
                    {{-- BUDGET MAKSIMUM --}}
                    <div>
                        <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">Budget Maksimum (Rp)</label>
                        <input type="number" name="price_max" value="{{ request('price_max') }}" placeholder="Contoh: 250000000"
                               class="w-full px-3.5 py-2 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:border-blue-600 focus:outline-none text-slate-800 placeholder-slate-400">
                    </div>

                    <button type="submit" class="w-full py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-semibold text-xs rounded-xl shadow-md transition">
                        Terapkan Filter
                    </button>
                </form>
            </aside>
                        ...
                        <div class="bg-white border border-slate-200/80 rounded-3xl shadow-sm hover:shadow-xl transition duration-300 overflow-hidden flex flex-col justify-between group">
                            ...
                            <div>
                                ...
                                <div class="relative aspect-[16/10] bg-slate-100 overflow-hidden">
                                    ...
                                    {{-- SKOR BADGE (RIGHT TOP) --}}
                                    <div class="absolute top-3 right-3 bg-slate-900/80 backdrop-blur-md text-white text-[10px] font-bold px-2.5 py-0.5 rounded-full flex items-center gap-1 border border-white/10">
                                        <span class="text-emerald-400">🛡️ Skor</span>
                                        <span>97/100</span>
                                    </div>
                            ...
                            <div class="p-5 pt-0 space-y-3">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <a href="https://wa.me/6281234567890?text={{ $waMsg }}" target="_blank" title="Chat via WhatsApp"
                    ...
                    @empty
                        <div class="col-span-full py-16 text-center bg-white rounded-3xl border border-slate-200">
                            ...
                            <p class="text-xs text-slate-500 mt-1 max-w-sm mx-auto">
                                Coba ubah kata kunci pencarian atau reset filter di bagian samping.
                            </p>
                        </div>
                    @endforelse
                </div>
